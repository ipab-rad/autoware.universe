#include "tartan_utils/point_type_converter.hpp"
#include <cmath>

namespace tartan_utils {

PointTypeConverter::PointTypeConverter(const rclcpp::NodeOptions & options) : Node("point_type_converter", options)
{
  subscription_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      "input", 10, std::bind(&PointTypeConverter::pointCloudCallback, this, std::placeholders::_1));
...
void PointTypeConverter::pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  for (size_t i = 0; i < msg->width * msg->height; ++i)
  {
    ...
    new_point.intensity = *reinterpret_cast<const uint8_t*>(&msg->data[i * msg->point_step + intensity_offset]);
    new_point.return_type = 1; // (TODO): Put the correct value
    new_point.channel = *reinterpret_cast<const uint16_t*>(&msg->data[i * msg->point_step + ring_offset]);
    new_point.distance = *reinterpret_cast<const float*>(&msg->data[i * msg->point_step + range_offset]);
    new_point.time_stamp = *reinterpret_cast<const uint32_t*>(&msg->data[i * msg->point_step + t_offset]);
    ...
    // new_point.elevation = atan2(new_point.z, sqrt(new_point.x * new_point.x + new_point.y * new_point.y));
...
RCLCPP_COMPONENTS_REGISTER_NODE(tartan_utils::PointTypeConverter)
